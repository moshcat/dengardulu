...
function generateCcidReport(analysis: FullAnalysis, lang: Lang): string {
  ...
  const flags = safety.red_flags
    .map((f, i) => `${i + 1}. ${lang === 'bm' ? f.label_bm : f.label_en} — "${f.evidence_quote}"`)
    .join('\n');

  return `LAPORAN PENIPUAN SUARA / VOICE SCAM REPORT
Dijana oleh DengarDulu pada ${date}
Generated by DengarDulu on ${date}

NOMBOR TELEFON / PHONE NUMBER: ${phone.queried_phone || 'Tidak dinyatakan'}
STATUS DALAM PANGKALAN DATA PDRM: ${phone.found ? `Ditemui — ${phone.report_count} laporan` : 'Tidak ditemui'}
SKOR SYAK WASANGKA / SUSPICION SCORE: ${safety.suspicion_score}/100 (${safety.verdict})

TRANSKRIP / TRANSCRIPT:
"${transcribe.transcript}"

BAHASA: ${transcribe.language}

BENDERA MERAH / RED FLAGS:
${flags}

PELAN TINDAKAN / ACTION PLAN:
${safety.action_plan.map((s, i) => `${i + 1}. ${lang === 'bm' ? s.step_bm : s.step_en}`).join('\n')}

---
Laporan ini dijana secara automatik oleh DengarDulu (https://dengardulu-169906713421.asia-southeast1.run.app).
Sila sahkan dengan pihak berkuasa.`;
}
...
function CcidReportDialog({ analysis, lang }: { analysis: FullAnalysis; lang: Lang }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <button onClick={() => setOpen(true)} className="outline-pill">
      ...
      <DialogContent showCloseButton>
        ...
        <DialogFooter showCloseButton={false}>
          ...
          >
            ...
            {copied ? t.report_copied : t.copy_report}
          </button>
          <a
            href="https://ereporting.rmp.gov.my/index.aspx/"
            target="_blank"
            rel="noopener noreferrer"
            className="outline-pill inline-flex items-center gap-2"
          >
